...
def create_counts(dct, mapping=None, verbose=True):
    """
        ...
        for key in tup[1]:
            for cnt in set(tup[1][key]):
                if tup[1][key].count(cnt) > 1:
                    n_mapping[hash('count.'+mapping[key]+'.'+str(cnt))] = 'count.'+mapping[key]+'.'+str(cnt)
                    n_mapping[hash(str(tup[1][key].count(cnt)))] = str(tup[1][key].count(cnt))
                    n_counts[hash('count.'+mapping[key]+'.'+str(cnt))] = [hash(str(tup[1][key].count(cnt)))]
            if len(tup[1][key]) > 1:
                n_mapping[hash('count.'+mapping[key])] = 'count.'+mapping[key]
                n_mapping[hash(str(len(tup[1][key])))] = str(len(tup[1][key]))
                n_counts[hash('count.'+mapping[key])] = [hash(str(len(tup[1][key])))]
        n = tup[1]
        n.update(n_counts)
        n_dct.append((tup[0], n))
    return n_dct, n_mapping
